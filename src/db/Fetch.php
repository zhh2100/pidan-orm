...
class Fetch
{
    ...
    protected function aggregate(string $aggregate, string $field): string
    {
        ...
        $field = $aggregate . '(' . $this->builder->parseKey($this->query, $field) . ') AS pidan_' . strtolower($aggregate);
    ...
    public function count(string $field = '*'): string
    {
        ...
        if (!empty($options['group'])) {
            // 支持GROUP
            $bind   = $this->query->getBind();
            $subSql = $this->query->options($options)->field('count(' . $field . ') AS pidan_count')->bind($bind)->buildSql();
