<?php
namespace pidan\db\builder;

use pidan\db\Builder;
...
class Sqlsrv extends Builder
{
     ...
     * @var string
     */
    protected $selectSql = 'SELECT T1.* FROM (SELECT pidanphp.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS pidanphp) AS T1 %LIMIT%%COMMENT%';
    /**
