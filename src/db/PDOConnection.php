...
use PDO;
use PDOStatement;
use pidan\db\exception\BindParamException;
use pidan\db\exception\DbEventException;
use pidan\db\exception\DbException;
use pidan\db\exception\PDOException;
use pidan\Model;
...
abstract class PDOConnection extends Connection
{
    ...
    {
        return $this->getConfig('builder') ?: '\\pidan\\db\\builder\\' . ucfirst($this->getConfig('type'));
    }
    ...
    public function aggregate(BaseQuery $query, string $aggregate, $field, bool $force = false)
    {
        ...
        $field = $aggregate . '(' . (!empty($distinct) ? 'DISTINCT ' : '') . $this->builder->parseKey($query, $field, true) . ') AS pidan_' . strtolower($aggregate);
